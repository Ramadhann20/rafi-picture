/* =========================================================
   LATE PAYMENT CONFIG
   ---------------------------------------------------------
   Ubah nilai di blok ini kalau kebijakan denda berubah.
========================================================= */
export const LATE_PAYMENT_CONFIG = Object.freeze({
  AMOUNT_PER_DAY: 100_000,

  /*
   * "floor" = denda bertambah setiap 24 jam penuh keterlambatan.
   * Contoh:
   * 23j 59m -> Rp0
   * 24j 00m -> Rp100.000
   * 48j 00m -> Rp200.000
   *
   * Ganti menjadi "ceil" jika setiap bagian hari ingin langsung
   * dihitung sebagai satu hari denda.
   */
  DAY_ROUNDING: "floor",

  /*
   * Invoice admin saat ini menyimpan dueAt sebagai YYYY-MM-DD.
   * Tenggat dianggap berakhir pukul 23:59:59 WIB.
   */
...
export function getEventStartDate(event) {
  ...
  if (
    ...
    const hour = timeMatch ? String(Number(timeMatch[1])).padStart(2, "0") : "00";
    const minute = timeMatch ? String(Number(timeMatch[2])).padStart(2, "0") : "00";
...
export function getSuggestedPenalty(overdueMs) {
  ...
  return {
    days,
    amount: days * LATE_PAYMENT_CONFIG.AMOUNT_PER_DAY,
  };
}
...
export function getInvoicePrincipalAmount(invoice) {
  ...
  return Math.max(
    0,
    (Number(invoice?.amount) || 0) - penaltyAmount,
  );
}
...
export function buildPaymentTimer({
  ...
  /*
   * Review pause dari revision sebelumnya dibawa ke invoice revision
   * berikutnya agar Apply Penalty tidak me-reset waktu keterlambatan.
   */
  ...
  const effectiveDeadlineMs =
    deadline.getTime() + totalReviewPauseMs;

  const pendingSubmittedAtMs = pendingPayment
    ? getTimestamp(
        pendingPayment?.submittedAt ?? pendingPayment?.createdAt,
      )
    : null;

  /*
   * Saat proof sedang diverifikasi admin, timer freeze pada saat submit.
   * Jika proof ditolak, waktu review itu menjadi pause dan timer lanjut
   * dari posisi sebelum review.
   */
